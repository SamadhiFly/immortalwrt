...
cfg_lte()
{
	...
	if [ -f "/etc/config/firewall" ]; then
		uci -q batch <<-EOF >/dev/null
			...
			set firewall.@redirect[-1].dest_ip='192.168.100.10'
	...
	sleep 1 && [ -f "/etc/init.d/mwan3" ] && /etc/init.d/mwan3 reload >/dev/null 2>&1 && sleep 1 && echo "info: service mwan3 reload done."

	sleep 1 && [ -f "/etc/init.d/checknet" ] && /etc/init.d/checknet reload >/dev/null 2>&1 && sleep 1 && echo "info: service checknet reload done."
